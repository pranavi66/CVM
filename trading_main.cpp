...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        cout << "Usage: trading script.tvm [--debug]" << endl;
        return 1;
    }
    bool debug = (argc >= 3 && string(argv[2]) == "--debug");
    try {
        string src    = readFile(argv[1]);
        auto tokens   = tokenize(src);          // same lexer
        auto ast      = parseTradingScript(tokens); // trading parser
        auto bc       = compile(*ast);          // same compiler
        if (debug) {
            cout << "=== Bytecode ===" << endl;
